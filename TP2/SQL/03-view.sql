...
AS
SELECT q.id_personne,
       p.nom,
       (TO_DATE('01-07-2021', 'DD-MM-YYYY') - (TO_DATE(q.date_debut, 'DD-MM-YYYY')) + 40)
...
WHERE (TO_DATE('01-07-2021', 'DD-MM-YYYY') - (TO_DATE(q.date_debut, 'DD-MM-YYYY')) + 40) > 0   -- borne exclusive
  AND (TO_DATE('01-07-2021', 'DD-MM-YYYY') - (TO_DATE(q.date_debut, 'DD-MM-YYYY')) + 40) <= 40 -- borne inclusive
...
-- ∆ entre les 2 dates en jours est <= 1 jour (bornée par 1 jour)
  AND p.id_personne = r.id_visiteur
...
CREATE OR REPLACE VIEW liste_entreprise_a_risque
AS
SELECT v.nom_entreprise,
       a.date_actuelle,
       p.id_personne,
       p.nom,
       p.adresse_numero,
       p.ville,
       es.date_heure_entree,
       es.date_heure_sortie,
       es.symptomes,
       es.temperature
FROM personne p
